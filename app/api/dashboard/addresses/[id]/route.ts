...
export async function PUT(
  ...
  try {
    ...
    if (isSecondaryId) {
      const secondaryPut = await fetch(`${wpBase}/wp-json/customers/v1/addresses-secondary/${addressId}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function DELETE(
  ...
  try {
    ...
    const isSecondaryId = addressId === 'billing2' || addressId === 'shipping2';
    if (isSecondaryId) {
      const secondaryDelete = await fetch(`${wpBase}/wp-json/customers/v1/addresses-secondary/${addressId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        cache: 'no-store',
      });
    ...
    if (!deleteResponse.ok) {
      ...
      try {
        ...
        if (err?.error) errorMessage = typeof err.error === 'string' ? err.error : err.error.message || errorMessage;
      } catch {
        // ignore
      }
