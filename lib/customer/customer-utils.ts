...
/** WordPress user id for the JWT (used when WC customer id matches WP user id). */
async function getWpUserNumericId(token: string): Promise<number | null> {
  const wpBase = getWpBaseUrl();
  if (!wpBase) return null;
  try {
